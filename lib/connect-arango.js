...
module.exports = function (connect) {
	...
	function ArangoStore(options, readyCallback) {
		...
		if (options.hash) {
			...
			this.hash = {};
			this.hash.salt = options.hash.salt
				? options.hash.salt
				: defaultSalt;
			this.hash.algorithm = options.hash.algorithm
		...
		if (
			options.stringify ||
			(!("stringify" in options) &&
				!("serialize" in options) &&
				!("unserialize" in options))
		) {
	...
	ArangoStore.prototype.get = function (sid, callback) {
		...
		if (
			time - this.db_clear_expires_time >=
			this.db_clear_expires_interval
		) {
			this.clearExpired();
