...
module.exports = function( connect ) {
     ...
     * Optional calls `readyCallback` when db connection is ready (mainly for testing purposes).
     *
    ...
    function ArangoStore( options, readyCallback ) {
        ...
        if (typeof options.db === 'object') {
            this.dbHelper = new DatabaseHelper(options.db, self.db_collection_name);
        } else {
        ...
        self.dbHelper.ensureCollection( self.db_collection_name, function( err, db ) {
            if (err) {
                ...
                db.index.createSkipListIndex( self.db_collection_name, [ "expires" ], false ).then( function( res ) {
                    ...
                    if (readyCallback) { readyCallback( err ); }
                } )
                    .catch( function( e ) {
                        throw e;
                    } );
    ...
    ArangoStore.prototype.clearExpired = function( callback ) {
        this.dbHelper.query()
            .exec( "FOR s IN SKIPLIST( " + this.db_collection_name + ", { expires: [[ '<', DATE_NOW() ]] } ) REMOVE s IN " + this.db_collection_name )
            .then( function( res ) {
                callback && callback();
            }, function( err ) {
